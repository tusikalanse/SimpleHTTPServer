...
template <int ObjectSize> class MyMemoryPool {
  union node {
    int offset;
...
template <int ObjectSize> void MyMemoryPool<ObjectSize>::set(int id) {
  if (IPCKEY == -1)
    return;
  if (id < 0 || id >= total)
    return;
...
// do nothing if IPCKEY == -1 or id is invalid
template <int ObjectSize> void MyMemoryPool<ObjectSize>::reset(int id) {
  if (IPCKEY == -1)
    return;
  if (id < 0 || id >= total)
    return;
...
int MyMemoryPool<ObjectSize>::init(int IPCKEY, int ObjectNumber) {
  int totalSize = sizeof(int) + 4 * (1 + (ObjectNumber - 1) / 32) +
                  ObjectSize * ObjectNumber;
  total = ObjectNumber;
...
template <int ObjectSize> MyMemoryPool<ObjectSize>::MyMemoryPool() {
  IPCKEY = shmid = -1;
}
...
template <int ObjectSize> void *MyMemoryPool<ObjectSize>::apply() {
  if (IPCKEY == -1)
    return NULL;
  if (*first == 0) {
    return NULL;
...
template <int ObjectSize> void *MyMemoryPool<ObjectSize>::get(int id) {
  if (IPCKEY == -1)
    return NULL;
  if (id < 0 || id >= total)
    return NULL;
...
template <int ObjectSize> int MyMemoryPool<ObjectSize>::getstatus(int id) {
  if (IPCKEY == -1)
    return -1;
  if (id < 0 || id >= total)
    return -1;
...
template <int ObjectSize> void MyMemoryPool<ObjectSize>::release(void *ptr) {
  if (IPCKEY == -1)
    return;
  if (ptr == NULL)
    return;
  int delta = static_cast<uint8_t *>(ptr) - static_cast<uint8_t *>(shmaddr);
  if ((static_cast<uint8_t *>(ptr) - reinterpret_cast<uint8_t *>(data)) %
          sizeof(node) !=
      0)
    return;
  if (*first == 0)
...
template <int ObjectSize> int MyMemoryPool<ObjectSize>::empty() {
  if (IPCKEY == -1)
    return -1;
  return count == 0;
}
...
template <int ObjectSize> int MyMemoryPool<ObjectSize>::size() {
  if (IPCKEY == -1)
    return -1;
  return count;
}
...
template <int ObjectSize> int MyMemoryPool<ObjectSize>::full() {
  if (IPCKEY == -1)
    return -1;
  return count == total;
}
...
template <int ObjectSize> void MyMemoryPool<ObjectSize>::clear() {
  if (IPCKEY == -1)
    return;
  for (int i = 0; i < 1 + (total - 1) / 8; ++i)
    used[i] = 0;
  *first =
      reinterpret_cast<uint8_t *>(data) - reinterpret_cast<uint8_t *>(first);
  for (int i = 0; i < total; ++i) {
    if (i != total - 1)
...
template <int ObjectSize> void MyMemoryPool<ObjectSize>::print() {
